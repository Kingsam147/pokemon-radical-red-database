...
export function resolveEnemyTeam(
  team: RawTeam,
  ...
  for (const [slotKey, entry] of Object.entries(team)) {
    if (!entry || slotKey === 'trainerInfo') {
      // At runtime this slot is always null or TrainerInfo — a RawPokemon
      // never sits under the 'trainerInfo' key — but the RawTeam union type
      // can't express that narrowing, so assert it explicitly.
...
export async function loadEnemyTeams(abilityList: Abilities, itemsList: Items, naturesList: Natures, movesList: PokemonMoves, typeList: PokemonTypes) {
  ...
  for (const [teamName, team] of Object.entries(enemyTeamsJSON.allTeams as Teams)) {
    resolvedP2Teams[teamName] = resolveEnemyTeam(
      team as unknown as RawTeam,
      abilityList,
      itemsList,
      naturesList,
      movesList,
