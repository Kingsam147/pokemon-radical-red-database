...
export async function fetchBoxCount(): Promise<number> {
  const res = await apiClient.get('/myBoxes/count');
...
export async function loadSingleBox(
  index: number,
  abilityList: Abilities,
  itemsList: Items,
  naturesList: Natures,
  ...
  return resolveSingleBox(rawBox, abilityList, itemsList, naturesList, movesList, typeList);
}
...
export async function addPokemon(boxIndex: string, pokemonData: string) {
  ...
  return {
    ...
    updatedBox: newPokemonJSON.updatedBox
  }
}
...
export function resolveBoxes(
    ...
    return rawBoxes.map(box => resolveSingleBox(box, abilityList, itemsList, naturesList, movesList, typeList));
}